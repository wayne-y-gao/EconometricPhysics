...
The econometric framing makes the separation precise. A DGP specification — $Y = f(X) + \varepsilon$, $\varepsilon \sim F$ — is a static law: it specifies both the structural function $f$ and the distribution of the innovation $\varepsilon$, but it does not produce any particular realization. *Running the model* — drawing $\varepsilon_1, \varepsilon_2, \ldots$ sequentially, computing each $Y_i$, advancing the filtration — is a separate act. The specification is time-symmetric; the running is not. QM is the specification: the finite complex sieve that determines the complete distributional structure of the information-layer process $f(x) + \varepsilon(x)$. The structural equation $f$ (GR) and the innovation process $\varepsilon$ (the second law) are both encoded in this specification, but they emerge at different stages — $f$ in the population limit, $\varepsilon$ at every finite realization. The thermodynamic process *runs the model*: it realizes each $\varepsilon_v$ along the causal order, Born-projects the specification-layer amplitude, and advances the filtration irreversibly. Measurement is estimation within this running process: conditioning on the filtration accumulated so far. The arrow of time is not a property of the specification (QM is reversible), nor of the structural equation (GR is reversible), but of the running — the sequential interface where the reversible law meets irreversible realization.
...
The characteristic function analogy illuminates, but the point where it breaks is equally instructive. The characteristic function $\varphi(t)$ and the density $f(x)$ are related by the Fourier transform, which is invertible: $\varphi$ determines $f$ and $f$ determines $\varphi$. The mapping is one-to-one. The characteristic function is an *equivalent representation* of the density — a complex-domain moment generating function that encodes the same distributional information in a different basis.

The wavefunction $\psi$ and the Born-projected probability density $|\psi|^2$ are not related this way. The Born map is many-to-one: multiplying $\psi$ by any phase $e^{i\theta}$ leaves $|\psi|^2$ unchanged. The density does not determine the wavefunction. The wavefunction is not an equivalent representation of the density but rather its physical generating mechanism — a complex-domain *probability density generating function*, if you will, that carries strictly more structure than the density it generates.

This asymmetry is why the specification layer is ontologically primitive, not merely a convenient redescription. If the characteristic function and the density are equivalent, then working in the Fourier domain is a matter of analytical convenience — an epistemic preference with no ontological content. But the wavefunction and the density are not equivalent: the Born projection is a genuine compression, not a change of basis. The phase that $|\psi|^2$ discards — the angle $\theta = \arctan(b/a)$ in the $\mathbb{R}^2$ representation — is real structure that the information layer cannot recover. Interference, entanglement, and Bell violation are empirical signatures of this extra structure: phenomena that the information layer can detect but cannot generate from its own resources. The quantum sieve representation is therefore an ontological postulate, not an equivalent description. It asserts the existence of structure that the Born-projected observables cannot reconstruct. Epistemology is ontology — but what can be identified at the information layer is strictly less than what exists at the specification layer. The specification layer is not a rephrasing of the density. It is the DGP itself.
...
The three are not coordinate objects — they are hierarchically related. The specification $\varphi$ encodes the complete DGP: both the structural function $f$ and the distribution of the innovation $\varepsilon$. The density and the innovations are what the specification produces when the model is run. The specification is the static law; $f(x) + \varepsilon(x)$ is the information-layer process it generates. In the econometric case, $\varphi$ and $f$ determine each other reversibly (via the Fourier transform and its inverse), so the hierarchy collapses: the specification is merely an equivalent encoding of the population target. The data generation is irreversible (once $\varepsilon$ is realized, the filtration grows and cannot shrink), but the specification and the population model are interchangeable.

No econometrician has ever seen a conflict between these three, because the hierarchy is transparent. $\varphi$ specifies the law; the law determines both $f$ (the population target) and the distribution of $\varepsilon$ (the stochastic input); running the model realizes the $\varepsilon$'s sequentially and advances the filtration. Each falls into its natural place: $\varphi$ at the specification layer, $f$ at the population layer, $\varepsilon$ at the information layer. There is no conflict because there was never a conflict — only a failure to recognize that three descriptions at three different layers of the same DGP are not three rival theories but one complete picture.

The same holds for GR, QM, and the second law — with the ontological stakes raised by the asymmetry noted above. QM is the specification: the finite complex sieve that encodes the complete DGP, determining both the structural equation $f$ (GR) and the distribution of the innovation process $\varepsilon$ (the second law). The specification is unitary and time-reversible because it is a law, not a process. It does not run the model. The thermodynamic innovation process runs it: realizing each $\varepsilon_v$ along the causal order, Born-projecting the specification-layer amplitude at each node, advancing the filtration irreversibly. Measurement and conditioning operate within this running process — an observer estimates local functionals from the Born-projected data accumulated in its filtration. GR emerges in the population limit of the running: the structural equation $f$ that the realized information-layer data identify and estimate as $n \to \infty$.

But unlike the econometric case, where $\varphi$ and $f$ are equivalent and the hierarchy collapses, the wavefunction and the Born-projected density are not equivalent: the specification layer carries phase, interference, and entanglement structure that the information layer cannot reconstruct. The hierarchy does not collapse. The specification layer is ontologically prior — it is the DGP that generates $f(x) + \varepsilon(x)$, not an alternative encoding of it. The three layers are genuinely distinct strata: QM specifies, thermodynamics runs, GR emerges.
